...
async function initDb(){if(!usePostgres)return;await pool.query(`CREATE TABLE IF NOT EXISTS vh_data (key text PRIMARY KEY, value jsonb NOT NULL)`);const r=await pool.query('SELECT key FROM vh_data');if(r.rowCount===0){const d=loadJson();for(const key of ['countries','packages','bookings','enquiries','reviews'])await pool.query('INSERT INTO vh_data(key,value) VALUES($1,$2)',[key,JSON.stringify(d[key]||[])]);console.log('PostgreSQL initialized from data/db.json')}}
async function getData(){if(!usePostgres)return loadJson();const r=await pool.query('SELECT key,value FROM vh_data');const d={countries:[],packages:[],bookings:[],enquiries:[],reviews:[]};for(const x of r.rows)d[x.key]=x.value;return d}
async function setData(d){if(!usePostgres){saveJson(d);return}for(const key of ['countries','packages','bookings','enquiries','reviews'])await pool.query('INSERT INTO vh_data(key,value) VALUES($1,$2) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value',[key,JSON.stringify(d[key]||[])]);}
...
app.post('/api/reviews',async(q,s)=>{
  ...
  let d=await getData(),x={id:nid(d.reviews),createdAt:new Date().toISOString(),name,phone,country,package:packageName,rating,text,verified:false,published:false,status:'pending'};
